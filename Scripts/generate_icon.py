...
def draw_sun(img):
    ...
    # Outer ring
    draw.ellipse(
        [vp_x - sun_r, vp_y - sun_r, vp_x + sun_r, vp_y + sun_r], fill=SUN_RING
    )
    # Mid ring
    draw.ellipse(
        [
    ...
    draw.ellipse(
        [vp_x - core_r, vp_y - core_r, vp_x + core_r, vp_y + core_r], fill=SUN_CORE
    )
...
def draw_road_reflection(img):
    ...
    for i in range(steps):
        ...
        rd.line([(vp_x - rw, y), (vp_x + rw, y)], fill=(*SUN_RING, alpha))
...
def draw_vignette(img):
    ...
    for i in range(steps):
        ...
        vd.ellipse(
            [SIZE // 2 - rx, SIZE // 2 - ry, SIZE // 2 + rx, SIZE // 2 + ry],
            outline=(0, 0, 0, alpha),
...
def main():
    print("ZenMap Icon Generator v3 — The Open Road")
