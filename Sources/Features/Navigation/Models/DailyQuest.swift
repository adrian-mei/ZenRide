...
class QuestStore: ObservableObject {
    ...
    private let storageKey = UserDefaultsKeys.questsV2
    ...
    private func save() {
        UserDefaults.standard.saveJSON(quests, forKey: storageKey)
    }

    private func load() {
        if let decoded = UserDefaults.standard.loadJSON([DailyQuest].self, forKey: storageKey) {
            quests = decoded
        } else {
            seedDefaults()
        }
