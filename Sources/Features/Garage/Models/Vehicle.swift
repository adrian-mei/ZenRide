...
struct VehicleTemplate: Identifiable, Equatable {
    ...
    static let all: [VehicleTemplate] = [
        ...
        // ── LEGENDARY (unlockLevel: 40+) ──────────────────────────────────
        VehicleTemplate(id: "magic_carpet", name: "Magic Carpet", make: "Mythic", model: "Weave", type: .scooter, unlockLevel: 40, speedStat: 9.0, handlingStat: 10.0, safetyStat: 1.0, colorHex: "D600FF"),
        VehicleTemplate(id: "ufo_saucer", name: "UFO Saucer", make: "Alien", model: "Saucer", type: .electricCar, unlockLevel: 50, speedStat: 10.0, handlingStat: 10.0, safetyStat: 10.0, colorHex: "00FF00")
    ]
}
...
class VehicleStore: ObservableObject {
    ...
    var selectedVehicleMode: VehicleMode {
        (VehicleTemplate.all.first { $0.id == selectedTemplateId } ?? VehicleTemplate.all[0]).type
    }
    ...
    private func save() {
        UserDefaults.standard.saveJSON(vehicles, forKey: storeKey)
    ...
    private func load() {
        ...
        if let decoded = UserDefaults.standard.loadJSON([Vehicle].self, forKey: storeKey) {
            vehicles = decoded
